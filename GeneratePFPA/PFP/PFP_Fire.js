...
function dynamicLoadFire(_url){
    for (let i = 0; i < 4; i++){
        let scriptTag = document.createElement('script');
        scriptTag.src = _url + `FR_${i+1}.js`;
        scriptTag.onload = function(){
            fires.texture[i] = darwDataImage(iData[`FR_${i+1}`]);
            if (i === 0){
                fires.sprite.texture = fires.texture[0];
                fires.sprite.texture.update();
            }
        };
        document.body.appendChild(scriptTag);
    }
    fires.sprite.on('pointerdown', pressCharacter);
    fires.sprite.interactive = true;
    fires.sprite.tint = 0xECF5FF;
    fires.sprite.anchor.set(0.5);
};
...
function setupImage() {
    ...
    fires = {texture:[]};
    fires.sprite = new PIXI.Sprite();
    characterGroup.addChild(eyes.sprite);
    characterGroup.addChild(fires.sprite);
...
function loadAndDraw(){
    ...
    dynamicLoadFire(`PFP/data/${PFP_T}/15_FR/`);
...
app.ticker.add((delta) => {
    ...
    let fid = Math.floor((eyeCount/10) % 4);
    //console.log(fid)
    fires.sprite.texture = fires.texture[fid];
    fires.sprite.texture.update()
    fires.sprite.position.set(FRiPos[0], head_y + FRiPos[1]);
